...
function PublicMenu({ db, setDbState, company, checkoutOnly = false }: { db: DatabaseApi; setDbState: React.Dispatch<React.SetStateAction<MockDatabaseState>>; company: Company; checkoutOnly?: boolean }) {
  ...
  function finishOrder() {
    ...
    setDbState((current) => ({
      ...
      customers: [
        ...current.customers,
        { id: customerId, company_id: company.id, name: checkout.name || "Cliente WhatsApp", phone: checkout.phone || "Não informado", address: checkout.address || "Retirada", total_spent: total, last_order_at: createdAt, created_at: createdAt },
      ],
      ...
      orders: [
        { id: orderId, company_id: company.id, customer_id: customerId, status: "novo", fulfillment, delivery_zone_id: fulfillment === "delivery" ? zoneId : undefined, subtotal, discount, delivery_fee: deliveryFee, total, payment_method: checkout.payment_method, created_at: createdAt },
        ...current.orders,
      ],
      order_items: [
    ...
    const message = [
      ...
      `Total: ${money(total)}`,
      fulfillment === "delivery" ? `Endereço: ${checkout.address}` : "Retirada no local",
    ].join("\n");
...
function AdminContent({ screen, db, setDbState, company, user }: { screen: AdminScreen; db: DatabaseApi; setDbState: React.Dispatch<React.SetStateAction<MockDatabaseState>>; company: Company; user: User }) {
  ...
  if (screen === "pedidos") return <OrdersManager bundle={bundle} setDbState={setDbState} company={company} />;
...
function OrdersManager({ bundle, setDbState, company }: { bundle: ReturnType<DatabaseApi["getCompanyBundle"]>; setDbState: React.Dispatch<React.SetStateAction<MockDatabaseState>>; company: Company }) {
...
function printOrder(company: Company, order: Order, bundle: ReturnType<DatabaseApi["getCompanyBundle"]>) {
  ...
  openPrintable("Pedido", `<h1>${company.name}</h1><p><strong>Pedido:</strong> ${order.id}</p><p><strong>Cliente:</strong> ${customerName(order.customer_id, bundle.customers)}</p><p><strong>Status:</strong> ${order.status}</p><table><tr><th>Item</th><th>Total</th></tr>${rows || "<tr><td colspan='2'>Itens não informados</td></tr>"}</table><p>Subtotal: ${money(order.subtotal)}</p><p>Entrega: ${money(order.delivery_fee)}</p><h2>Total: ${money(order.total)}</h2><p class="signature">Startt Delivery — produzido por Startt Facilities</p>`);
}

function sendOrderUpdate(order: Order, customer: string, company: Company) {
  const message = [`${company.name}`, `Pedido: ${order.id}`, `Cliente: ${customer}`, `Status atual: ${order.status}`, `Total: ${money(order.total)}`].join("\n");
