...
const adminNav: Array<{ id: AdminScreen; label: string; icon: React.ReactNode }> = [
  ...
  { id: "relatorios", label: "RelatÃ³rios", icon: <BarChart3 size={18} /> },
  { id: "fretes", label: "Fretes", icon: <MapPin size={18} /> },
  { id: "impressao", label: "ImpressÃ£o", icon: <Printer size={18} /> },
  { id: "configuracoes", label: "ConfiguraÃ§Ãµes", icon: <Settings size={18} /> },
  { id: "usuarios", label: "UsuÃ¡rios", icon: <UserRound size={18} /> },
];
...
function runSave(setSaving: (value: boolean) => void, action: () => void, success: string) {
  ...
  window.setTimeout(() => {
    try {
      ...
      notify("error", "NÃ£o foi possÃ­vel salvar agora. Confira os dados e tente novamente.");
    } finally {
      setSaving(false);
    }
  }, SAVE_DELAY);
...
function openPrintable(title: string, html: string) {
  ...
  if (!popup) {
    notify("error", "NÃ£o foi possÃ­vel abrir a janela de impressÃ£o. Verifique o bloqueador de pop-ups.");
    return;
  }
  ...
  notify("success", "ImpressÃ£o preparada. Confirme a impressÃ£o na janela aberta.");
}
...
function App() {
  ...
  if (!company) return withToast(<NotFound message={`NÃ£o encontramos a empresa â€œ/${parts[0]}â€. Confira o link e tente novamente.`} />);
    ...
    <main className="min-h-screen bg-startt-paper">
      ...
      <section className="mx-auto grid w-[min(1180px,calc(100%-32px))] gap-6 py-10">
        <div>
          ...
          <p className="mt-4 max-w-2xl text-lg leading-8 text-startt-muted">CardÃ¡pio pÃºblico por slug, admin por empresa e Admin Master para gestÃ£o do SaaS.</p>
        </div>
          ...
          {companies.map((company) => (
            <a key={company.id} href={`/${company.slug}`} className="overflow-hidden rounded-lg border border-black/10 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
              ...
              <div className="grid gap-2 p-5">
                ...
                <span className="inline-flex items-center gap-2 text-sm font-bold text-startt-green">Abrir cardÃ¡pio <ChevronRight size={16} /></span>
              </div>
            </a>
          ))}
        </div>
...
function PublicMenu({ db, setDbState, company, checkoutOnly = false }: { db: DatabaseApi; setDbState: React.Dispatch<React.SetStateAction<MockDatabaseState>>; company: Company; checkoutOnly?: boolean }) {
  ...
  function finishOrder() {
    ...
    if (fulfillment === "delivery" && (!checkout.address.trim() || !zoneId)) {
      notify("error", "Informe endereÃ§o e bairro de entrega.");
      return;
    }
    ...
    setDbState((current) => ({
      ...
      customers: [
        ...current.customers,
        { id: customerId, company_id: company.id, name: checkout.name || "Cliente WhatsApp", phone: checkout.phone || "NÃ£o informado", address: fullAddress, total_spent: total, last_order_at: createdAt, created_at: createdAt },
      ],
    ...
    const message = [
      `OlÃ¡, ${company.name}! Quero fazer um pedido:`,
      "",
      ...
      {!checkoutOnly && (
        <section className="relative isolate flex min-h-[520px] items-end overflow-hidden px-4 py-10 md:px-10">
          ...
          <div className="mx-auto w-[min(1280px,100%)] pb-4 text-white">
            <span className="inline-flex items-center gap-2 text-sm font-black uppercase text-startt-yellow"><Sparkles size={16} /> CardÃ¡pio pÃºblico</span>
            <h1 className="mt-3 max-w-4xl text-5xl font-black leading-none md:text-8xl">{company.name}</h1>
            <p className="mt-4 max-w-2xl text-lg leading-8 text-white/90">{company.opening_hours} â€¢ {company.estimated_delivery_time}</p>
        ...
        <section>
          <div className="mb-5 flex flex-col justify-between gap-3 md:flex-row md:items-end">
            <div>
              ...
              <h2 className="text-3xl font-black">{checkoutOnly ? "Checkout" : "CardÃ¡pio"}</h2>
            </div>
          ...
          {!checkoutOnly && (
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              ...
              {!filtered.length && <div className="rounded-lg border border-black/10 bg-white p-6 text-startt-muted md:col-span-2 xl:col-span-3">Nenhum produto encontrado neste cardÃ¡pio.</div>}
            </div>
          )}
        </section>
      </section>
...
function CartDrawer({ cartOpen, setCartOpen, cart, setCart, company, zones, zoneId, setZoneId, checkout, setCheckout, fulfillment, setFulfillment, subtotal, discount, deliveryFee, total, finishOrder }: { cartOpen: boolean; setCartOpen: (value: boolean) => void; cart: CartItem[]; setCart: React.Dispatch<React.SetStateAction<CartItem[]>>; company: Company; zones: DeliveryZone[]; zoneId: string; setZoneId: (value: string) => void; checkout: CheckoutState; setCheckout: (value: CheckoutState) => void; fulfillment: Fulfillment; setFulfillment: (value: Fulfillment) => void; subtotal: number; discount: number; deliveryFee: number; total: number; finishOrder: () => void }) {
  ...
  async function lookupCep(value: string) {
    ...
    try {
      ...
      if (data.erro) {
        setCepMessage("CEP nÃ£o encontrado. VocÃª pode preencher o endereÃ§o manualmente.");
        return;
      }
      ...
      if (matchedZone) {
        ...
        setZoneId("");
        setCepMessage("Seu bairro ainda nÃ£o possui frete cadastrado. VocÃª pode selecionar manualmente.");
      }
    } catch {
      setCepMessage("NÃ£o foi possÃ­vel buscar o CEP agora. Preencha manualmente.");
    } finally {
      setCepLoading(false);
    }
  }
    ...
    <div className={`fixed inset-0 z-50 ${cartOpen ? "pointer-events-auto" : "pointer-events-none"}`}>
      ...
      <aside className={`absolute right-0 top-0 grid h-full w-[min(460px,100%)] grid-rows-[auto_1fr_auto] bg-white shadow-2xl transition-transform ${cartOpen ? "translate-x-0" : "translate-x-full"}`}>
        ...
        <div className="overflow-auto p-5">
          {cart.length === 0 ? <Empty text="Seu pedido ainda estÃ¡ vazio." /> : cart.map((item) => (
        ...
        <div className="grid gap-3 border-t border-black/10 bg-startt-paper p-5">
          ...
          {fulfillment === "delivery" && <>
            ...
            <div className="grid grid-cols-2 gap-2">
              <Input placeholder="NÃºmero" value={checkout.number} onChange={(value) => setCheckout({ ...checkout, number: value })} />
          ...
          <div className="grid grid-cols-2 gap-2">
            <Select value={checkout.payment_method} onChange={(value) => setCheckout({ ...checkout, payment_method: value as PaymentMethod })}><option>Pix</option><option>CartÃ£o</option><option>Dinheiro</option></Select>
            <Input placeholder="Cupom" value={checkout.coupon} onChange={(value) => setCheckout({ ...checkout, coupon: value })} />
          </div>
          {checkout.payment_method === "Dinheiro" && <div className="grid gap-2 rounded-lg border border-black/10 bg-white p-3"><Input type="number" placeholder="Troco para quanto?" value={checkout.cash_change_for} onChange={(value) => setCheckout({ ...checkout, cash_change_for: value })} />{cashChangeFor > 0 && <span className="text-sm font-bold text-startt-muted">Troco para {money(cashChangeFor)} â€¢ Troco: {money(calculatedChange)}</span>}</div>}
...
function CompanyAdmin({ db, setDbState, company, screen, login }: { db: DatabaseApi; setDbState: React.Dispatch<React.SetStateAction<MockDatabaseState>>; company: Company; screen: AdminScreen; login: boolean }) {
  ...
  function announceNewOrder() {
    ...
    try {
      ...
      // Navegadores podem bloquear Ã¡udio sem interaÃ§Ã£o prÃ©via.
    }
  ...
  function doLogin(event: React.FormEvent) {
    ...
    if (!found) {
      setLoginError("Login invÃ¡lido para esta empresa.");
      notify("error", "E-mail ou senha invÃ¡lidos para esta empresa.");
      return;
    }
    setLoginError("");
  ...
  function logout() {
    ...
    notify("info", "VocÃª saiu do painel.");
    ...
    <main className="min-h-screen bg-startt-paper">
      <header className="sticky top-0 z-30 border-b border-black/10 bg-startt-paper/95 px-4 py-3 backdrop-blur-xl">
        <div className="mx-auto flex w-[min(1280px,100%)] flex-wrap items-center justify-between gap-3">
          <LogoTitle title="Startt Delivery" subtitle={`${company.name} â€¢ ${user.name} (${user.role})`} />
          <div className="flex flex-wrap items-center gap-2">
            ...
            <a className="rounded-lg border border-black/10 bg-white px-4 py-3 font-extrabold" href={`/${company.slug}`}>Ver cardÃ¡pio</a>
          </div>
        </div>
      </header>
...
function PlanBlocked() {
  return <section className="rounded-lg border border-black/10 bg-white p-6"><h1 className="text-3xl font-black">Recurso indisponÃ­vel</h1><p className="mt-2 text-startt-muted">Seu plano atual nÃ£o inclui este recurso. Entre em contato com a Startt Facilities.</p></section>;
}
...
function Dashboard({ company, bundle }: { company: Company; bundle: ReturnType<DatabaseApi["getCompanyBundle"]> }) {
  ...
  function pdf() {
    ...
    openPrintable("RelatÃ³rio Dashboard", `<h1>${company.name}</h1><p>PerÃ­odo: ${start} atÃ© ${end}</p><p>Total de vendas: ${money(total)}</p><p>Quantidade de pedidos: ${online.length}</p><p>Ticket mÃ©dio: ${money(total / Math.max(1, all.length))}</p><h2>Pedidos por status</h2><ul>${byStatus}</ul><p class="signature">Startt Delivery â€” produzido por Startt Facilities</p>`);
  }
  ...
  return (
    <section className="grid gap-5">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Metric label="Vendas do mÃªs" value={money(salesMonth)} icon={<BarChart3 />} />
        <Metric label="Pedidos" value={String(online.length)} icon={<ClipboardList />} />
        <Metric label="Ticket mÃ©dio" value={money(total / Math.max(1, all.length))} icon={<FileText />} />
...
function Cashier({ company, user, products, setDbState }: { company: Company; user: User; products: Product[]; setDbState: React.Dispatch<React.SetStateAction<MockDatabaseState>> }) {
  ...
  function finish() {
    ...
    setCart([]);
    setDiscount(0);
    openPrintable("Comprovante", `<h1>${company.name}</h1><p>Venda presencial ${sale.id}</p><p>Total: ${money(total)}</p><p class="signature">Startt Delivery â€” produzido por Startt Facilities</p>`);
  ...
  return (
    <section className="grid gap-5 lg:grid-cols-[1fr_360px]">
      ...
      <Panel title="Carrinho do caixa">
        ...
        <Select value={payment} onChange={(value) => setPayment(value as PaymentMethod)}><option>Pix</option><option>CartÃ£o</option><option>Dinheiro</option></Select>
...
function OrdersManager({ bundle, setDbState, company, user }: { bundle: ReturnType<DatabaseApi["getCompanyBundle"]>; setDbState: React.Dispatch<React.SetStateAction<MockDatabaseState>>; company: Company; user: User }) {
  ...
  function remove(order: Order) {
    if (!["dono", "gerente"].includes(user.role)) {
      notify("error", "Seu usuÃ¡rio nÃ£o tem permissÃ£o para excluir pedidos.");
      return;
    }
    ...
    notify("success", "Pedido excluÃ­do com sucesso.");
  }
  return (
    <CrudShell title="Pedidos" description="Pedidos recebidos do cardÃ¡pio online.">
...
function sendOrderUpdate(order: Order, customer: string, company: Company) {
  ...
  notify("success", "Mensagem de atualizaÃ§Ã£o preparada para WhatsApp.");
}
...
function ProductsManager({ company, products, categories, plan, setDbState }: { company: Company; products: Product[]; categories: Category[]; plan?: Plan; setDbState: React.Dispatch<React.SetStateAction<MockDatabaseState>> }) {
  ...
  function save(event: React.FormEvent) {
    ...
    if (!form.name.trim() || !positiveNumber(form.price)) {
      notify("error", "Informe nome e preÃ§o vÃ¡lido para salvar o produto.");
      return;
    }
    if (!form.category_id) {
  ...
  function remove(product: Product) { if (confirm(`Excluir ${product.name}?`)) { setDbState((current) => ({ ...current, products: current.products.filter((item) => !(item.id === product.id && item.company_id === company.id)) })); notify("success", "Produto excluÃ­do com sucesso."); } }
  return (
    <CrudShell title="Produtos" description="Cadastrar, editar, excluir e ativar/desativar produtos.">
      <form onSubmit={save} className="grid gap-3 rounded-lg border border-black/10 bg-white p-4">
        <div className="grid gap-3 md:grid-cols-3"><Input placeholder="Nome" value={form.name} onChange={(value) => setForm({ ...form, name: value })} /><Input placeholder="PreÃ§o" value={form.price} onChange={(value) => setForm({ ...form, price: value })} /><Select value={form.category_id} onChange={(value) => setForm({ ...form, category_id: value })}>{categories.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</Select></div>
        ...
        <textarea className="min-h-24 rounded-lg border border-black/10 px-3 py-3 outline-startt-green" placeholder="DescriÃ§Ã£o" value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} />
        ...
        <button disabled={saving} className="w-fit rounded-lg bg-startt-green px-4 py-3 font-black text-white disabled:opacity-60">{saving ? "Salvando..." : form.id ? "Salvar alteraÃ§Ãµes" : "Criar produto"}</button>
      </form>
      <Table headers={["Produto", "Categoria", "PreÃ§o", "Status", "AÃ§Ãµes"]} rows={products.map((product) => [product.name, categoryName(product.category_id, categories), money(product.price), product.active ? "Ativo" : "Inativo", <Actions key={product.id} onEdit={() => edit(product)} onDelete={() => remove(product)} />])} />
    </CrudShell>
  );
}
...
function CategoriesManager({ company, categories, setDbState }: { company: Company; categories: Category[]; setDbState: React.Dispatch<React.SetStateAction<MockDatabaseState>> }) {
  ...
  function remove(category: Category) { if (confirm(`Excluir ${category.name}?`)) { setDbState((current) => ({ ...current, categories: current.categories.filter((item) => !(item.id === category.id && item.company_id === company.id)) })); notify("success", "Categoria excluÃ­da com sucesso."); } }
  return <CrudShell title="Categorias" description="Cadastrar, editar ordem e ativar/desativar categorias."><InlineAdd value={name} setValue={setName} onAdd={add} placeholder="Nova categoria" /><Table headers={["Nome", "Ordem", "Status", "AÃ§Ãµes"]} rows={categories.map((category) => [<Input key={category.id} value={category.name} placeholder="Categoria" onChange={(value) => setDbState((current) => ({ ...current, categories: current.categories.map((item) => item.id === category.id ? { ...item, name: value } : item) }))} />, String(category.sort_order), category.active ? "Ativa" : "Inativa", <div key={category.id} className="flex gap-2"><button className="rounded-lg border px-3 py-2 font-bold" onClick={() => toggle(category)}>Ativar/desativar</button><button className="rounded-lg bg-startt-red px-3 py-2 font-bold text-white" onClick={() => remove(category)}>Excluir</button></div>])} /></CrudShell>;
}
...
function CustomersManager({ company, customers, setDbState }: { company: Company; customers: Customer[]; setDbState: React.Dispatch<React.SetStateAction<MockDatabaseState>> }) {
  ...
  return <CrudShell title="Clientes" description="Cadastro manual, ediÃ§Ã£o, exclusÃ£o e histÃ³rico."><form onSubmit={save} className="grid gap-3 rounded-lg border border-black/10 bg-white p-4 md:grid-cols-4"><Input placeholder="Nome" value={form.name} onChange={(value) => setForm({ ...form, name: value })} /><Input placeholder="Telefone" value={form.phone} onChange={(value) => setForm({ ...form, phone: value })} /><Input placeholder="EndereÃ§o" value={form.address} onChange={(value) => setForm({ ...form, address: value })} /><button className="rounded-lg bg-startt-green px-4 font-black text-white">{form.id ? "Salvar" : "Cadastrar"}</button></form><Table headers={["Cliente", "Telefone", "EndereÃ§o", "Total gasto", "Ãšltimo pedido", "AÃ§Ãµes"]} rows={customers.map((customer) => [customer.name, customer.phone, customer.address, money(customer.total_spent), customer.last_order_at ? customer.last_order_at.slice(0, 10) : "-", <Actions key={customer.id} onEdit={() => setForm({ id: customer.id, name: customer.name, phone: customer.phone, address: customer.address })} onDelete={() => confirm("Excluir cliente?") && setDbState((current) => ({ ...current, customers: current.customers.filter((item) => !(item.id === customer.id && item.company_id === company.id)) }))} />])} /></CrudShell>;
}
...
function CouponsManager({ company, coupons, plan, setDbState }: { company: Company; coupons: Coupon[]; plan?: Plan; setDbState: React.Dispatch<React.SetStateAction<MockDatabaseState>> }) {
  ...
  const [form, setForm] = useState(blank);
  function add() { if (!form.code || !positiveNumber(form.value)) { notify("error", "Informe cÃ³digo e valor vÃ¡lido para o cupom."); return; } if (plan && !plan.allow_coupons) { notify("error", "Seu plano atual nÃ£o inclui este recurso. Entre em contato com a Startt Facilities."); return; } setDbState((current) => ({ ...current, coupons: [{ id: id("cup"), company_id: company.id, code: form.code.toUpperCase(), type: form.type as Coupon["type"], value: Number(form.value), minimum_order: Number(form.minimum_order), usage_limit: Number(form.usage_limit), used_count: 0, expires_at: form.expires_at, active: true }, ...current.coupons] })); setForm(blank); notify("success", "Cupom criado com sucesso."); }
  ...
  return <CrudShell title="Cupons" description="Criar, editar, excluir e ativar/desativar cupons."><div className="grid gap-3 rounded-lg border border-black/10 bg-white p-4 md:grid-cols-6"><Input placeholder="CÃ³digo" value={form.code} onChange={(value) => setForm({ ...form, code: value })} /><Select value={form.type} onChange={(value) => setForm({ ...form, type: value })}><option value="percentual">Percentual</option><option value="fixo">Valor fixo</option></Select><Input placeholder="Valor" value={form.value} onChange={(value) => setForm({ ...form, value })} /><Input placeholder="Pedido mÃ­nimo" value={form.minimum_order} onChange={(value) => setForm({ ...form, minimum_order: value })} /><Input placeholder="Limite" value={form.usage_limit} onChange={(value) => setForm({ ...form, usage_limit: value })} /><button onClick={add} className="rounded-lg bg-startt-green px-4 font-black text-white">Criar</button></div><Table headers={["CÃ³digo", "Tipo", "Valor", "MÃ­nimo", "Uso", "Status", "AÃ§Ãµes"]} rows={coupons.map((coupon) => [coupon.code, coupon.type, coupon.type === "percentual" ? `${coupon.value}%` : money(coupon.value), money(coupon.minimum_order), `${coupon.used_count}/${coupon.usage_limit}`, coupon.active ? "Ativo" : "Inativo", <div key={coupon.id} className="flex gap-2"><button className="rounded-lg border px-3 py-2 font-bold" onClick={() => toggle(coupon)}>Ativar/desativar</button><button className="rounded-lg bg-startt-red px-3 py-2 font-bold text-white" onClick={() => setDbState((current) => ({ ...current, coupons: current.coupons.filter((item) => !(item.id === coupon.id && item.company_id === company.id)) }))}>Excluir</button></div>])} /></CrudShell>;
}
...
function PrintManager({ company, settings, setDbState }: { company: Company; settings?: PrintSettings; setDbState: React.Dispatch<React.SetStateAction<MockDatabaseState>> }) {
  const fallback: PrintSettings = { company_id: company.id, auto_print_orders: false, auto_print_cash_sales: false, printer_name: "", paper_width: "80mm", copies: 1, footer_text: "Startt Delivery â€” produzido por Startt Facilities" };
  ...
  return <CrudShell title="ImpressÃ£o" description="Modo normal usa window.print(). Futuro modo avanÃ§ado pode usar QZ Tray."><div className="grid gap-3 rounded-lg border border-black/10 bg-white p-4 md:grid-cols-2"><label className="flex gap-2 font-bold"><input type="checkbox" checked={form.auto_print_orders} onChange={(e) => setForm({ ...form, auto_print_orders: e.target.checked })} /> Imprimir novos pedidos</label><label className="flex gap-2 font-bold"><input type="checkbox" checked={form.auto_print_cash_sales} onChange={(e) => setForm({ ...form, auto_print_cash_sales: e.target.checked })} /> Imprimir vendas do caixa</label><Input placeholder="Nome da impressora" value={form.printer_name} onChange={(value) => setForm({ ...form, printer_name: value })} /><Select value={form.paper_width} onChange={(value) => setForm({ ...form, paper_width: value as "58mm" | "80mm" })}><option>58mm</option><option>80mm</option></Select><Input placeholder="Quantidade de vias" value={String(form.copies)} onChange={(value) => setForm({ ...form, copies: Number(value) || 1 })} /><Input placeholder="RodapÃ©" value={form.footer_text} onChange={(value) => setForm({ ...form, footer_text: value })} /><button onClick={save} className="rounded-lg bg-startt-green px-4 py-3 font-black text-white">Salvar configuraÃ§Ã£o</button><button onClick={() => openPrintable("Teste de impressÃ£o", `<h1>${company.name}</h1><p>Teste ${form.paper_width}</p><p>${form.footer_text}</p>`)} className="rounded-lg border border-black/10 bg-white px-4 py-3 font-black">Testar impressÃ£o</button></div></CrudShell>;
}
...
function CompanySettings({ company, setDbState }: { company: Company; setDbState: React.Dispatch<React.SetStateAction<MockDatabaseState>> }) {
  ...
  const [saving, setSaving] = useState(false);
  function save() { if (saving) return; runSave(setSaving, () => setDbState((current) => ({ ...current, companies: current.companies.map((item) => item.id === company.id ? { ...form, hero_image: form.banner_url || form.hero_image } : item) })), "ConfiguraÃ§Ãµes salvas com sucesso."); }
  return <CrudShell title="ConfiguraÃ§Ãµes" description="Essas configs alteram o cardÃ¡pio pÃºblico da empresa."><div className="grid gap-3 rounded-lg border border-black/10 bg-white p-4 md:grid-cols-2"><Input placeholder="Nome da empresa" value={form.name} onChange={(value) => setForm({ ...form, name: value })} /><Input placeholder="Logo URL" value={form.logo_url} onChange={(value) => setForm({ ...form, logo_url: value })} /><Input placeholder="Banner URL" value={form.banner_url || form.hero_image} onChange={(value) => setForm({ ...form, banner_url: value, hero_image: value })} /><Input placeholder="WhatsApp" value={form.whatsapp} onChange={(value) => setForm({ ...form, whatsapp: value })} /><Input placeholder="EndereÃ§o" value={form.address} onChange={(value) => setForm({ ...form, address: value })} /><Input placeholder="Pedido mÃ­nimo" value={String(form.minimum_order)} onChange={(value) => setForm({ ...form, minimum_order: Number(value) || 0 })} /><Input placeholder="Tempo estimado" value={form.estimated_delivery_time} onChange={(value) => setForm({ ...form, estimated_delivery_time: value })} /><Input placeholder="Cor principal" value={form.primary_color} onChange={(value) => setForm({ ...form, primary_color: value })} /><Input placeholder="HorÃ¡rio de funcionamento" value={form.opening_hours} onChange={(value) => setForm({ ...form, opening_hours: value })} /><label className="flex gap-2 font-bold"><input type="checkbox" checked={form.is_open} onChange={(e) => setForm({ ...form, is_open: e.target.checked })} /> Aberto</label><label className="flex gap-2 font-bold"><input type="checkbox" checked={form.delivery_enabled} onChange={(e) => setForm({ ...form, delivery_enabled: e.target.checked })} /> Permitir entrega</label><label className="flex gap-2 font-bold"><input type="checkbox" checked={form.pickup_enabled} onChange={(e) => setForm({ ...form, pickup_enabled: e.target.checked })} /> Permitir retirada</label><Input placeholder="Mensagem de rodapÃ©" value={form.footer_message} onChange={(value) => setForm({ ...form, footer_message: value })} /><button disabled={saving} onClick={save} className="w-fit rounded-lg bg-startt-green px-4 py-3 font-black text-white disabled:opacity-60">{saving ? "Salvando..." : "Salvar configuraÃ§Ãµes"}</button></div></CrudShell>;
}
...
function UsersManager({ company, users, plan, setDbState }: { company: Company; users: User[]; plan?: Plan; setDbState: React.Dispatch<React.SetStateAction<MockDatabaseState>> }) {
  ...
  const [form, setForm] = useState(blank);
  const [saving, setSaving] = useState(false);
  function add() { if (saving) return; if (!form.name || !form.email || !form.password) { notify("error", "Informe nome, e-mail e senha do usuÃ¡rio."); return; } if (!company.is_registration_enabled) { notify("error", "Cadastro/acesso temporariamente desativado."); return; } if (plan && users.length >= plan.max_users) { notify("error", "Limite de usuÃ¡rios do plano atingido. Entre em contato com a Startt Facilities."); return; } runSave(setSaving, () => { setDbState((current) => ({ ...current, users: [{ id: id("usr"), company_id: company.id, name: form.name, email: form.email, password: form.password, role: form.role, is_active: true, created_at: new Date().toISOString() }, ...current.users] })); setForm(blank); }, "UsuÃ¡rio criado com sucesso."); }
  function toggle(user: User) { runSave(setSaving, () => setDbState((current) => ({ ...current, users: current.users.map((item) => item.id === user.id && item.company_id === company.id ? { ...item, is_active: !item.is_active } : item) })), user.is_active ? "UsuÃ¡rio bloqueado com sucesso." : "UsuÃ¡rio desbloqueado com sucesso."); }
...
function MasterApp({ db, setDbState, screen, login }: { db: DatabaseApi; setDbState: React.Dispatch<React.SetStateAction<MockDatabaseState>>; screen: MasterScreen; login: boolean }) {
  ...
  function doLogin(event: React.FormEvent) { event.preventDefault(); const found = db.master_users.find((user) => user.email === credentials.email && user.password === credentials.password && user.is_active); if (!found) { setMasterError("Login master invÃ¡lido ou bloqueado."); return; } setMasterError(""); localStorage.setItem(MASTER_SESSION_KEY, found.id); setSession(found.id); window.history.pushState({}, "", "/master"); }
...
function MasterContent({ screen, db, setDbState }: { screen: MasterScreen; db: DatabaseApi; setDbState: React.Dispatch<React.SetStateAction<MockDatabaseState>> }) {
  ...
  if (screen === "configuracoes") return <CrudShell title="ConfiguraÃ§Ãµes SaaS" description="ConfiguraÃ§Ãµes globais preparadas para evoluÃ§Ã£o."><Panel title="Ambiente">Startt Delivery â€” produzido por Startt Facilities</Panel></CrudShell>;
  ...
  return <section className="grid gap-5"><AdminHero title="Dashboard geral" description="MÃ©tricas gerais do SaaS Startt Delivery." /><div className="grid gap-4 md:grid-cols-4"><Metric label="Total de empresas" value={String(db.companies.length)} icon={<Building2 />} /><Metric label="Empresas ativas" value={String(db.companies.filter((item) => item.status === "active").length)} icon={<Check />} /><Metric label="Empresas trial" value={String(db.companies.filter((item) => item.status === "trial").length)} icon={<Sparkles />} /><Metric label="Inadimplentes" value={String(db.companies.filter((item) => item.subscription_status === "overdue").length)} icon={<CreditCard />} /><Metric label="Bloqueadas" value={String(db.companies.filter((item) => item.status === "blocked").length)} icon={<ShieldCheck />} /><Metric label="Pedidos no sistema" value={String(db.orders.length)} icon={<ClipboardList />} /><Metric label="MRR previsto" value={money(mrr)} icon={<CreditCard />} /></div><Panel title="Ãšltimos restaurantes cadastrados">{db.companies.slice(-5).map((company) => <div key={company.id} className="flex justify-between border-b py-2"><span>{company.name}</span><b>{company.status} â€¢ {company.subscription_status}</b></div>)}</Panel></section>;
}
...
function MasterCompanies({ db, setDbState }: { db: DatabaseApi; setDbState: React.Dispatch<React.SetStateAction<MockDatabaseState>> }) {
  ...
  function saveCompany(event: React.FormEvent) {
    ...
    if (!form.name || !form.slug || !form.whatsapp || !form.address || !form.plan_id || !form.primary_color) {
      notify("error", "Preencha nome, slug, WhatsApp, endereÃ§o, plano e cor principal.");
      return;
    }
    if (!isValidSlug(form.slug)) {
      notify("error", "Use um slug simples, com letras minÃºsculas, nÃºmeros e hÃ­fen. Exemplo: pizzariadojoao.");
      return;
    }
    if (!positiveNumber(form.monthly_price)) {
      notify("error", "Informe um valor mensal vÃ¡lido.");
      return;
    }
...
function MasterUsers({ db, setDbState }: { db: DatabaseApi; setDbState: React.Dispatch<React.SetStateAction<MockDatabaseState>> }) {
  ...
  return <CrudShell title="UsuÃ¡rios" description="UsuÃ¡rios de todas as empresas, com filtro, criaÃ§Ã£o, bloqueio e redefiniÃ§Ã£o de senha."><div className="grid gap-3 rounded-lg border bg-white p-4 md:grid-cols-6"><Select value={form.company_id} onChange={(value) => setForm({ ...form, company_id: value })}>{db.companies.map((company) => <option key={company.id} value={company.id}>{company.name}</option>)}</Select><Input placeholder="Nome" value={form.name} onChange={(value) => setForm({ ...form, name: value })} /><Input placeholder="E-mail" value={form.email} onChange={(value) => setForm({ ...form, email: value })} /><Input placeholder="Senha" value={form.password} onChange={(value) => setForm({ ...form, password: value })} /><Select value={form.role} onChange={(value) => setForm({ ...form, role: value as UserRole })}>{(["dono", "gerente", "caixa", "atendente"] as UserRole[]).map((role) => <option key={role}>{role}</option>)}</Select><button onClick={addUser} className="rounded-lg bg-startt-green px-4 font-black text-white">Criar usuÃ¡rio</button></div><Select value={companyId} onChange={setCompanyId}><option value="todos">Todas as empresas</option>{db.companies.map((company) => <option key={company.id} value={company.id}>{company.name}</option>)}</Select><Table headers={["Nome", "E-mail", "Empresa", "FunÃ§Ã£o", "Status", "AÃ§Ãµes"]} rows={users.map((user) => [user.name, user.email, db.companies.find((c) => c.id === user.company_id)?.name || "-", user.role, user.is_active ? "Ativo" : "Bloqueado", <div key={user.id} className="flex gap-2"><button className="rounded-lg border px-3 py-2 font-bold" onClick={() => setDbState((current) => ({ ...current, users: current.users.map((item) => item.id === user.id ? { ...item, is_active: !item.is_active } : item) }))}>Ativar/bloquear</button><button className="rounded-lg border px-3 py-2 font-bold" onClick={() => { const password = prompt("Nova senha", "123456"); if (password) setDbState((current) => ({ ...current, users: current.users.map((item) => item.id === user.id ? { ...item, password } : item) })); }}>Redefinir senha</button></div>])} /></CrudShell>;
}
...
function MasterPlans({ db, setDbState }: { db: DatabaseApi; setDbState: React.Dispatch<React.SetStateAction<MockDatabaseState>> }) {
  ...
  function save(event: React.FormEvent) {
    ...
    if (!form.name || !positiveNumber(form.monthly_price) || !positiveNumber(form.max_products) || !positiveNumber(form.max_users)) {
      notify("error", "Informe nome, preÃ§o e limites vÃ¡lidos para o plano.");
      return;
    }
  ...
  function remove(plan: Plan) {
    if (db.companies.some((company) => company.plan_id === plan.id)) { notify("error", "Plano em uso nÃ£o pode ser excluÃ­do."); return; }
    if (!confirm(`Excluir plano ${plan.name}?`)) return;
    setDbState((current) => ({ ...current, plans: current.plans.filter((item) => item.id !== plan.id) }));
    notify("success", "Plano excluÃ­do com sucesso.");
  }
  return <CrudShell title="Planos" description="Criar, editar, desativar e excluir planos nÃ£o usados."><form onSubmit={save} className="grid gap-3 rounded-lg border bg-white p-4"><div className="grid gap-3 md:grid-cols-5"><Input placeholder="Nome" value={form.name} onChange={(value) => setForm({ ...form, name: value })} /><Input placeholder="PreÃ§o mensal" value={form.monthly_price} onChange={(value) => setForm({ ...form, monthly_price: value })} /><Input placeholder="MÃ¡x. produtos" value={form.max_products} onChange={(value) => setForm({ ...form, max_products: value })} /><Input placeholder="MÃ¡x. usuÃ¡rios" value={form.max_users} onChange={(value) => setForm({ ...form, max_users: value })} /><button className="rounded-lg bg-startt-green px-4 font-black text-white">{form.id ? "Salvar plano" : "Criar plano"}</button></div><div className="flex flex-wrap gap-4"><label className="flex gap-2 font-bold"><input type="checkbox" checked={form.allow_reports} onChange={(event) => setForm({ ...form, allow_reports: event.target.checked })} /> RelatÃ³rios</label><label className="flex gap-2 font-bold"><input type="checkbox" checked={form.allow_printing} onChange={(event) => setForm({ ...form, allow_printing: event.target.checked })} /> ImpressÃ£o</label><label className="flex gap-2 font-bold"><input type="checkbox" checked={form.allow_coupons} onChange={(event) => setForm({ ...form, allow_coupons: event.target.checked })} /> Cupons</label><label className="flex gap-2 font-bold"><input type="checkbox" checked={form.is_active} onChange={(event) => setForm({ ...form, is_active: event.target.checked })} /> Ativo</label></div></form><Table headers={["Plano", "PreÃ§o", "Produtos", "UsuÃ¡rios", "Recursos", "Status", "AÃ§Ãµes"]} rows={db.plans.map((plan) => [plan.name, money(plan.monthly_price), String(plan.max_products), String(plan.max_users), `${plan.allow_reports ? "RelatÃ³rios " : ""}${plan.allow_printing ? "ImpressÃ£o " : ""}${plan.allow_coupons ? "Cupons" : ""}` || "BÃ¡sico", plan.is_active ? "Ativo" : "Inativo", <div key={plan.id} className="flex gap-2"><button className="rounded-lg border px-3 py-2 font-bold" onClick={() => edit(plan)}>Editar</button><button className="rounded-lg border px-3 py-2 font-bold" onClick={() => setDbState((current) => ({ ...current, plans: current.plans.map((item) => item.id === plan.id ? { ...item, is_active: !item.is_active } : item) }))}>Ativar/desativar</button><button className="rounded-lg bg-startt-red px-3 py-2 font-bold text-white" onClick={() => remove(plan)}>Excluir</button></div>])} /></CrudShell>;
}
      ...
      <section className="relative grid max-h-[92vh] w-[min(720px,100%)] overflow-hidden rounded-lg bg-white shadow-2xl animate-in zoom-in-95 duration-200 md:grid-cols-[280px_1fr]">
        ...
        <div className="grid gap-4 overflow-auto p-5">
          ...
          <Panel title="Ingredientes">
            <p className="text-sm leading-6 text-startt-muted">{product.ingredients || "Ingredientes nÃ£o informados."}</p>
          </Panel>
...
function Suspended({ company, publicView = false, message }: { company: Company; publicView?: boolean; message?: string }) { return <main className="grid min-h-screen place-items-center bg-startt-paper p-4"><section className="max-w-lg rounded-lg border bg-white p-6 text-center shadow-xl"><h1 className="text-3xl font-black">Acesso suspenso</h1><p className="mt-3 text-startt-muted">{message || `${company.name} estÃ¡ com acesso suspenso. Entre em contato com a Startt Facilities.`} {publicView ? "O cardÃ¡pio estÃ¡ temporariamente indisponÃ­vel." : ""}</p></section></main>; }
